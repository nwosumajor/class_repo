...
function CheckoutForm({ orderId, total }: { orderId: string, total: number }) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Send the payment_method_id to YOUR backend (File 11/16)
      const response = await fetch(
        `${apiBaseUrl.replace('/auth', '')}/orders/${orderId}/pay`, // This is /api/orders/:id/pay
        {
          method: 'POST',
          headers: {
      ...
      // 3. Payment Success!
      clearCart(); // Clear the thrift cart
      // router.push(`/thrift/order/success?orderId=${orderId}`); // <-- REPLACED
      window.location.href = `/thrift/order/success?orderId=${orderId}`; // <-- FIXED
...
// --- NEW HOOK to get URL params on the client ---
function useUrlParams() {
  const [params, setParams] = useState<{ orderId: string | null, total: string | null }>({ orderId: null, total: null });
...
function PaymentUI() {
  ...
  const { orderId, total } = useUrlParams(); // <-- REPLACED
      ...
      <Card>
        ...
        <CardContent>
          ...
          <Button asChild variant="link" className="p-0 mt-2">
            <a href="/thrift/cart">Return to Thrift Cart</a>
          </Button>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-2xl font-bold text-center">Pay for Thrift Item</CardTitle>
 ...
 */
export default function ThriftPaymentPage() {
  return (
